...
func (db *Client) UpsertL1BatchForBaseAll(ctx context.Context, batches []*ethclient.BaseBatchEvent) (*uint64, error) {
	if len(batches) == 0 {
		return nil, nil
	}
	...
	for i, batch := range batches {
		batchNumber, err = db.UpsertL1BatchForBase(ctx, batch)
		if err != nil {
			return nil, err
		}
...
func (db *Client) UpsertL1BatchForBase(ctx context.Context, batch *ethclient.BaseBatchEvent) (*uint64, error) {
	// Insert batch
	sqlStmt := `
