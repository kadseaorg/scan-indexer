...
func bindLinea(address common.Address, caller bind.ContractCaller, transactor bind.ContractTransactor, filterer bind.ContractFilterer) (*bind.BoundContract, error) {
	parsed, err := abi.JSON(strings.NewReader(LineaABI))
	if err != nil {
		return nil, err
	}
...
// LineaBlockFinalizedIterator is returned from FilterBlockFinalized and is used to iterate over the raw logs and unpacked data for BlockFinalized events raised by the Linea contract.
type LineaBlockFinalizedIterator struct {
	Event *LineaBlockFinalized // Event containing the contract specifics and raw log
...
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *LineaBlockFinalizedIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	...
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(LineaBlockFinalized)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
...
// Error returns any retrieval or parsing error occurred during filtering.
func (it *LineaBlockFinalizedIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *LineaBlockFinalizedIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// LineaBlockFinalized represents a BlockFinalized event raised by the Linea contract.
type LineaBlockFinalized struct {
	BlockNumber   *big.Int
...
func (_Linea *LineaFilterer) FilterBlockFinalized(opts *bind.FilterOpts, blockNumber []*big.Int, stateRootHash [][32]byte) (*LineaBlockFinalizedIterator, error) {
	...
	logs, sub, err := _Linea.contract.FilterLogs(opts, "BlockFinalized", blockNumberRule, stateRootHashRule)
	if err != nil {
		return nil, err
	}
	return &LineaBlockFinalizedIterator{contract: _Linea.contract, event: "BlockFinalized", logs: logs, sub: sub}, nil
}
...
//
// Solidity: event BlockFinalized(uint256 indexed blockNumber, bytes32 indexed stateRootHash)
func (_Linea *LineaFilterer) WatchBlockFinalized(opts *bind.WatchOpts, sink chan<- *LineaBlockFinalized, blockNumber []*big.Int, stateRootHash [][32]byte) (event.Subscription, error) {
	...
	logs, sub, err := _Linea.contract.WatchLogs(opts, "BlockFinalized", blockNumberRule, stateRootHashRule)
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(LineaBlockFinalized)
				if err := _Linea.contract.UnpackLog(event, "BlockFinalized", log); err != nil {
					return err
				}
				event.Raw = log
...
//
// Solidity: event BlockFinalized(uint256 indexed blockNumber, bytes32 indexed stateRootHash)
func (_Linea *LineaFilterer) ParseBlockFinalized(log types.Log) (*LineaBlockFinalized, error) {
	event := new(LineaBlockFinalized)
	if err := _Linea.contract.UnpackLog(event, "BlockFinalized", log); err != nil {
		return nil, err
	}
