...
func (i *Indexer) StartL1IndexerForLinea(worker int, forceStartBlock *uint64) {
	...
	if forceStartBlock != nil {
		...
		lastSyncedBlockNumber, err = i.dbClient.GetLatestBatchOfLastL1ProveBlock(i.ctx)
		if err != nil {
	...
	last_l2_block_number, err := i.dbClient.GetLatestBatchOfLastL2FinalizedBlock(i.ctx)
	if err != nil {
		log.Error().Msgf("Error getting last l2 block number: %s", err)
		return
	}

	if last_l2_block_number == 0 {
		last_l2_block_number = 2238199
	}
	...
	for {
		...
		for {
			...
			log.Info().Msgf("🤷 [L1 Indexer] Filter event between: [%d, %d] %d", startBlock, endBlock, lastSyncedBlockNumber)

			batchNumber, batchesLen, err := i.indexL1BatchesForLinea(&last_l2_block_number, startBlock, endBlock)
			if err != nil {
...
func (i *Indexer) WaitingWriteBatchForLinea(batchEvents []*ethclient.LineaBatchEvent) error {
	...
	for _, event := range batchEvents {
		for _, l2block := range event.FinalizedBlockNumbers {
...
func (i *Indexer) indexL1BatchesForLinea(last_l2_block_number *uint64, from, to uint64) (int64, int, error) {
	batchEvents, err := i.l1client.GetLineaBatchEventsFromL1(i.ctx, last_l2_block_number, from, to)
	if err != nil {
	...
	if err := i.WaitingWriteBatchForLinea(batchEvents); err != nil {
		return 0, 0, fmt.Errorf("error waiting for block to be finalized: %w", err)
	}

	batchNumber, err := i.dbClient.UpsertL1BatchForLineaAll(i.ctx, batchEvents)
	if err != nil {
