...
func (i *Indexer) StartL1IndexerForBase(worker int, forceStartBlock *uint64) {
	...
	for {
		...
		for {
			...
			batchNumber, batchesLen, err := i.indexL1BatchesForBase(startBlock, endBlock)
			if err != nil {
...
func (i *Indexer) indexL1BatchesForBase(from uint64, to uint64) (uint64, int, error) {
	batchEvents, err := i.l1client.GetBaseOutputProposedEventLogs(i.ctx, from, to)
	if err != nil {
	...
	if err := i.WaitingWriteBatchForBase(batchEvents); err != nil {
		return 0, 0, fmt.Errorf("error waiting for block to be finalized: %w", err)
	}

	batchNumber, err := i.dbClient.UpsertL1BatchForBaseAll(i.ctx, batchEvents)
	if err != nil {
...
func (i *Indexer) WaitingWriteBatchForBase(batchEvents []*ethclient.BaseBatchEvent) error {
	...
	for _, e := range batchEvents {
		if e.L2BlockNumber <= latestIndexedBlockNumber {
			continue
		}
		for {
			...
			log.Debug().Msgf("🚧 Waiting for l2 block %d to be indexed, so we can write batch info", e.L2BlockNumber)
		}
	}
