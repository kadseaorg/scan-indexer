...
func (i *Indexer) StartL1IndexerForOro(worker int, forceStartBlock *uint64) {
	...
	for {
		...
		for {
			...
			batchNumber, batchesLen, err := i.indexL1BatchesForOro(startBlock, endBlock)
			if err != nil {
...
func (i *Indexer) indexL1BatchesForOro(from uint64, to uint64) (uint64, int, error) {
	batchEvents, err := i.l1client.GetOroOutputProposedEventLogs(i.ctx, from, to)
	if err != nil {
	...
	if err := i.WaitingWriteBatchForOro(batchEvents); err != nil {
		return 0, 0, fmt.Errorf("error waiting for block to be finalized: %w", err)
	}

	batchNumber, err := i.dbClient.UpsertL1BatchForOroAll(i.ctx, batchEvents)
	if err != nil {
...
func (i *Indexer) WaitingWriteBatchForOro(batchEvents []*ethclient.BaseBatchEvent) error {
