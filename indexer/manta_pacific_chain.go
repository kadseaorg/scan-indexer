...
func (i *Indexer) StartL1IndexerForMantaPacific(worker int, forceStartBlock *uint64) {
	...
	for {
		...
		for {
			...
			batchNumber, batchesLen, err := i.indexL1BatchesForMantaPacific(startBlock, endBlock)
			if err != nil {
...
func (i *Indexer) WaitingWriteBatchForMantaPacific(batchEvents []*ethclient.MantaPacificBatchEvent) error {
	...
	for _, event := range batchEvents {
		for _, l2block := range event.FinalizedBlockNumbers {
			...
			for {
				...
				log.Debug().Msgf("🚧 Waiting for l2 block %d to be indexed, so we can write batch info", l2block.Uint64())
			}
		}
	}
...
func (i *Indexer) indexL1BatchesForMantaPacific(from uint64, to uint64) (uint64, int, error) {
	batchEvents, err := i.l1client.GetMantaPacificOutputProposedEventLogs(i.ctx, from, to)
	if err != nil {
	...
	if err := i.WaitingWriteBatchForMantaPacific(batchEvents); err != nil {
		return 0, 0, fmt.Errorf("error waiting for block to be finalized: %w", err)
	}

	batchNumber, err := i.dbClient.UpsertL1BatchForMantaPacificAll(i.ctx, batchEvents)
	if err != nil {
